...
export const CheckoutPage = () => {
  ...
  const handleCheckout = (email: string, address: string) => {
    const newOrder = createOrder(product.id, email, address);
    setOrder(newOrder);
  };
  return <div className="container mx-auto px-4 py-8">
      <Link to={`/product/${id}`} className="inline-flex items-center text-emerald-600 hover:text-emerald-700 mb-6">
          ...
          <div className="flex flex-col md:flex-row">
            <div className="md:w-1/2 md:pr-6">
              ...
              {!order ? <>
                  ...
                  <CheckoutForm productId={product.id} onSubmit={handleCheckout} />
            ...
            <div className="md:w-1/2 md:pl-6 mt-6 md:mt-0">
              {order ? <>
                  ...
                  <StratumInstructions stratumUrl={order.stratumUrl} workerName={order.workerName} progress={progress} />
                  {progress >= 100 && <div className="mt-6 text-center">
                      <p className="text-gray-700 mb-4">
                        Mining complete! Your order is being processed.
                      </p>
                      <button onClick={() => navigate('/')} className="px-6 py-2 bg-emerald-600 text-white font-medium rounded-md hover:bg-emerald-700">
                        Continue Shopping
                      </button>
                    </div>}
